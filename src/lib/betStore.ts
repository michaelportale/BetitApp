...
// In-memory storage with AsyncStorage persistence (replace with Supabase in production)
export class BetStore {
  ...
  getGroupByInviteCode(inviteCode: string): Group | undefined {
  ...
  submitProof(betId: string, _userId: string, _proof: string): void {
    const bet = this.bets.get(betId);
  ...
  addTestParticipants(betId: string): boolean {
    ...
    const availableUsers = group.memberIds.filter(
      id => id !== bet.creatorId && !bet.participants.some(p => p.userId === id)
    );
  ...
  async loadFromStorage(): Promise<boolean> {
    try {
      ...
      const ledgerData = values.find(([key]) => key === this.STORAGE_KEYS.LEDGER)?.[1];
      const initializedData = values.find(([key]) => key === this.STORAGE_KEYS.INITIALIZED)?.[1];
      ...
      if (groupsData) {
        ...
        this.groups.forEach(group => {
          this.groupMembers.set(group.id, new Set(group.memberIds));
        });
      }
  ...
  async markAsInitialized() {
    await AsyncStorage.setItem(this.STORAGE_KEYS.INITIALIZED, 'true');
  }
}
...
// Auto-initialize with persistence support
async function initializeStore() {
  try {
    ...
    if (!hasExistingData) {
      ...
      initializeMockData();
      await betStore.markAsInitialized();
      console.log('Mock data initialized and saved successfully');
    } else {
